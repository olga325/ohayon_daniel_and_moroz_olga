...
eco <-eco[order(eco$`GDP, PPP (current international $)` , decreasing = TRUE),]
top_5 <- head(eco,5)
top_5

lowest_GDP <- round(nrow(eco)%*% 0.1,)
eco <- head(eco,-lowest_GDP)

plot(x = as.numeric(eco$pop65), y = log(as.numeric(eco$`GDP, PPP (current international $)`)), xlab="65+ percent", ylab="GDP per capita" ,main="GDP - POP 65+")

abline(lm(log(as.numeric(eco$`GDP, PPP (current international $)`)) ~ as.numeric(eco$pop65))) 
...
join3 <- left_join(corona, eco, by = "country" )
```
...
plot(x =join_cig$Average...taxes.as.a...of.cigarette.price...total.tax, y = as.numeric(join_cig$normal_pop),xlim = c(0,100), ylim = c(0,0.005) ,xlab="% taxes of Cigarette Price ", ylab="Number of cases" ,main="Conection between Cigarette Taxes and Number of Cases")
