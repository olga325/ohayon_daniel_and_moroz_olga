...
isTRUE(all.equal(cases.agg, cases.agg2))
isTRUE(all.equal(deaths.agg, deaths.agg2))
isTRUE(all.equal(recovered.agg, recovered.agg2))
#d
...
join_corona <- head(join_corona[order(join_corona$Value.x , decreasing = TRUE),],10)

barp_table <- data.frame(name = rep(c("sick","deaths","recovered"), each = 10), country = rep(join_corona$Country.Region,3),values = c(join_corona$sick, join_corona$Value.y, join_corona$Value))

colors = c("orange","red","green")
val_mat <- matrix(c(barp_table$values),nrow = 3,ncol = 10,byrow = TRUE)
countries <- as.vector(join_corona$Country.Region)
...
#d
eco$GDP_per_capita <- c(as.numeric(eco$GDP ) / as.numeric(eco$pop_total))
...
plot(x = as.numeric(eco_plot$pop65), y = log(as.numeric(eco_plot$GDP_per_capita)), xlab="65+ percent", ylab="GDP per capita" ,main="GDP - POP 65+")
abline(lm(log(as.numeric(eco_plot$GDP_per_capita)) ~ as.numeric(eco_plot$pop65))) 
...
corona_eco$cases <- c((as.numeric(corona_eco$Value.x)/as.numeric(corona_eco$pop_total)) * 100000)
corona_eco$deaths <- c((as.numeric(corona_eco$Value.y)/as.numeric(corona_eco$pop_total))* 100000)
corona_eco$recovered <- c((as.numeric(corona_eco$Value)/as.numeric(corona_eco$pop_total)) * 100000)
...
corona_eco_march <- corona_eco_march[which(as.numeric(corona_eco_march$pop65) >= 15),]
corona_eco_march$color <- "black"
corona_eco_march$color[corona_eco_march$pop65 > 24] = "red"
corona_eco_march$color[corona_eco_march$deaths > 15] = "red"
                       
plot(x = log10(as.numeric(corona_eco_march$Value.x)), y = as.numeric(corona_eco_march$pop65), xlab="deaths and cases", ylab="pop 65" ,main="1") + points(x =log10(as.numeric(corona_eco_march$Value.y)), y = as.numeric(corona_eco_march$pop65),type="p",col = corona_eco_march$color)
# abline(lm(log10(as.numeric(corona_eco_march$pop65))~ as.numeric(corona_eco_march))) 
```
...
join_cig$normal_pop <- c((as.numeric(join_cig$Value) / as.numeric(join_cig$pop_total)*100))

# Creates a graph that examines the connection between cig.taxes and the corona cases number in each country
plot(x =as.numeric(join_cig$Average...taxes.as.a...of.cigarette.price...total.tax), y = as.numeric(join_cig$normal_pop),xlim = c(0,100), ylim = c(0,0.015) ,xlab="% taxes of Cigarette Price ", ylab="% Corona cases from Population - * 100" ,main="Conection between Cigarette Taxes and Number of Cases")
abline(lm(as.numeric(join_cig$normal_pop)) ~ as.numeric(join_cig$Average...taxes.as.a...of.cigarette.price...total.tax)) 


```
The research question is whether there is a connection between government policy on smoking and the number of corona cases relative to the country's population. Our assumption was that the more aggressive the cigarette tax the government could reduce the risk of being infected with Corona.
