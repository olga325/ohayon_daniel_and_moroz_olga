...
join2$sick <- c(join2$Value.x-join2$Value.y-join2$Value)
...
barplot(as.matrix(join2), main = "Top 10 ", names.arg = as.character(join2$Country.Region), xlab = "countries", ylab = "total cases", col = c("red","orange","blue"))
...
plot(log(as.numeric(eco_wide$`GDP, PPP (current international $)`)),as.numeric(eco_wide$pop65), xlab="65+", ylab="GDP",main="plot")

# GDP.lm <- lm(log(as.numeric(eco_wide$`GDP, PPP (current international $)`)) ~ as.numeric(eco_wide$pop65)) + abline(GDP.lm)
```
