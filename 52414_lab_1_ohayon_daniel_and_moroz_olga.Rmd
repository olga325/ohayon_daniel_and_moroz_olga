...
**Solution:**   
```{r}
corona <- left_join(deaths.agg, cases.agg, by= c("Country.Region", "Date"))
corona <- left_join (corona, recovered.agg, by = c("Country.Region", "Date"))
names(corona)[names(corona) == "Country.Region"] <- "country"
join3 <- left_join(eco,corona, by = "country" )
#which(corona$country=="Russia")
#which(eco$country=="Russian Federation")
check<- anti_join(eco,corona,by="country")
eco_names <- as.character(eco$country[!eco$country %in% corona$country])
corona_names <- as.character(unique(corona$country))
print(length(corona_names)==length(eco_names)) # not the same number of countries in corona and eco, missing information/
eco_names
eco_pltnames <- as.character(eco$country[!eco$country %in% eco_plot$country])
eco_pltnames
...
plot(x =join_cig$Average...taxes.as.a...of.cigarette.price...total.tax, y = as.numeric(join_cig$normal_pop),xlim = c(0,100), ylim = c(0,0.015) ,xlab="% taxes of Cigarette Price ", ylab="% Corona cases from Population - * 100" ,main="Conection between Cigarette Taxes and Number of Cases")
